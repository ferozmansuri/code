...
class BF_Model extends CI_Model
{
    ...
    public function insert($data = null)
    {
		...
		//firoz :- changing current member variables temporary to skip display error and exception.
		...
		if ($status == false) {
            ...
            $this->error_logger->log("FB lead campaign - Insert query failed: ".$this->get_db_error_message(), 0);
        } elseif ($this->return_insert_id) {
            $id = $this->db->insert_id();

            $status = $this->trigger('after_insert', $id);
        }
		//firoz :- resetting of member variables.
